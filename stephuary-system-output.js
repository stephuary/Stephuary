...
(function (global) {
  ...
  function weekActionsFromS2() {
    var el = global.document.getElementById('out-s7');
    if (!el) el = global.document.getElementById('out-s6');
  ...
  function mountRevenue(container, ctaHtml) {
    ...
    var ignore = truncate(textById('out-s5'), 280);
    ...
    var leverageAction = textById('out-s6');
    ...
    var posLine = textById('out-s6');
    var audience = truncate(textById('out-s3'), 220);

    var fastestHead = truncate(textById('out-s3'), 320);
